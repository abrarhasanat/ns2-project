...
opt3 = ['agileSD' , 'compound' , 'cubic' , 'highspeed']
opt1 = [('#flow','#Flow',flow),('#node','#Node',node),('packet_per_second','#Packet',node), ('queue' , '#Queue', queue)]
# opt0 = ['taskA2/']
...
for k in opt3:
    for i in opt1:
        ...
        for idx, j in enumerate(y[0]):
            ...
            plt.xlabel(i[0])
            plt.ylabel(y[0][idx])
            plt.title(y[0][idx] + " vs " + y[0][0] + " in " + k )
            plt.grid()

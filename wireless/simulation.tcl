...
if { $argc != 6 } {
    puts "Usage: ns $argv0 <number of nodes> <number of flows> <number of packets/sec> <area size> "
    exit 1
}
...
set val(ifqlen)       [lindex $argv 4]                      ;# max packet in ifq
...
for {set i 0} {$i <= $rows} {incr i} {
    ...
    for {set j 0} {$j < $jmax } {incr j} {
        ...
        $node($node_no) random-motion 0       ;# disable random motion
